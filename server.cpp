...
int main()
{
    ...
    serverAdder.sin_addr.s_addr = inet_addr(SERVER_IP);
    ...
    if( bind(listener,(struct sockaddr*)&serverAdder,sizeof(serverAdder))<0)
    {
        perror("bind error");
        exit(-1);
    }
    ...
    printf("epoll created,epollfa = %d\n",epfd);
    ...
    while(1)
    {
       ...
       for(size_t i = 0; i < epoll_events_count; ++i)
       {
            ...
            if(sockfd == listener)
            {
                ...
                printf("Now there are %d client int the chat room \n",(int) clients_list.size());
